...
export interface Issue {
  ...
  required?: string;
  /**
   * Structured fix data for `verdict fix`: the exact CSS property to change
   * and the exact value to change it to. Populated directly from the same
   * computation that produces `suggestedFix`'s human sentence -- never
   * derived by parsing that sentence back apart, which would be fragile.
   * Absent for issues that aren't safely auto-fixable (e.g. missing <h1>,
   * missing alt text -- those need generated content, not a value swap).
   */
  fixProperty?: string;
  fixValue?: string;
}
