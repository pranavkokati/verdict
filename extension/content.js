...
(function () {
  ...
  // Shared visibility test -- see the matching comment in
  // src/render/browser.ts's extractInPage(). Applied to elements,
  // headings, landmarks, and images alike so "present in this snapshot"
  // means "perceivable," not just "exists in the DOM."
  ...
  function extractFromLiveDocument() {
    ...
    // never <body> or <html> themselves -- so a page's background color set
    // directly on <body> (the most common way to set a page's background,
    // and essential for dark-themed pages) was invisible to
    // findEffectiveBackground below. Ported fix from src/render/browser.ts.
  ...
  // Composites every containing background layer (outermost first, matching
  // paint order) instead of returning only the nearest one's raw color --
  // that dropped a semi-transparent layer's alpha entirely (relativeLuminance
  // only reads r/g/b), treating e.g. rgba(255,255,255,0.5) as opaque white.
  // Ported fix from src/checks/contrast.ts's findEffectiveBackground.
  function findEffectiveBackground(el, all) {
    ...
    let composited = parseColor("rgb(255,255,255)");
